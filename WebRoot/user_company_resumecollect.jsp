...
<html>
  ...
  <%
  List<Resumecollect> list = new ArrayList();
  Employer em = (Employer)session.getAttribute("em");
  String filmid = em.getFilmid();
  if(filmid!=null){
  ResumeDao dao = new ResumeDao();
  list = dao.selectcollectfromfilmid(filmid);
  }
   %>
  <body>
    <!--页眉-->
...
<div class="big_nav">
	<div class="nav clearfix">
		...
		<div class="nav_a clearfix">
			...
			<a class="in" href="user_company_resumecollect.jsp">我的收藏</a>
		</div>
	</div>
</div>
...
<div class="big_czmm_content">
	<div class="denglu_content">
		...
		<div class="content3_l">
			...
			<%
			for(Resumecollect res : list){
			 %>
			
			<a class="box mr_0 clearfix" href="Copy of user_company_resume.jsp?filmid=<%=em.getFilmid() %>&realname=<%=res.getRealname() %>&postid=<%=res.getPostid()%>&userid=<%=res.getUsername()%>">
				<div class="text">
					<b><%=res.getRealname() %></b>
