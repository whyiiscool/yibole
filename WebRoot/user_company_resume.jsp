  ...
  res = dao.selectfrompostid(filmid, realname, postid);
   ...
   <script language="javascript">
	function firm() {//利用对话框返回的值 （true 或者 false）
		if (confirm("确定保存这份简历？")) {//如果是true ，那么就把页面转向https://blog.csdn.net/h2503652646
			location.href = "colletresumeaction?filmid=<%=res.getFilmid() %>&realname=<%=res.getRelname() %>&postid=<%=res.getPostid()%>&userid=<%=res.getRelname()%>";
			alert("保存成功。");
			}
		...
		<div class="content1_l">
			<div class="biaoti_01 clearfix">
				...
				<div class="jlfen clearfix">
					...
					<a class="sxjl" onclick="firm()">收藏简历</a>
				</div>
			</div>
