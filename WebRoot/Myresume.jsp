...
<html>
  ...
  Employee em = (Employee)session.getAttribute("ep");
  String username = em.getUsername();
  System.out.print(username);
  ResumeDao dao = new ResumeDao();
  list = dao.selectfromname(username);
...
<div class="big_czmm_content">
	<div class="denglu_content">
		...
		<div class="content3_l">
			...
			<a class="box mr_0 clearfix" href="lokeresume.jsp?id=<%=res.getId() %>&relname=<%=res.getRelname() %>&username=<%=res.getUsername() %>
							 	&filmid=<%=res.getFilmid() %>&postid=<%=res.getPostid() %>&sex=<%=res.getSex() %>&experience=<%=res.getExperience() %>&
							 	education=<%=res.getEducation() %>&email=<%=res.getEmail() %>&usernum=<%=res.getUsernum() %>&liveplace=<%=res.getLiveplace() %>&brith=<%=res.getBrith() %>&age=<%=res.getAge() %>">
				<div class="text">
					<b><%=res.getRelname() %></b>
